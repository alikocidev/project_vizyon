...
const Discover = ({ auth, shows }: DiscoverProps) => {
  ...
  const [searchQuery, setSearchQuery] = useState<string>((route().params?.s as string) || "");
  ...
  const handleClickMore = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (searchQuery.trim().toLowerCase().length >= 3) {
      SearchNewThings(FilterValues.show_type, searchQuery.trim().toLowerCase(), newPage)
        .then((newShows) => {
  ...
  return (
    <>
      <CoreLayout user={auth.user} title="Vizyondakiler" expand>
        <div className="w-full h-full flex flex-col">
          <div className="z-[51] w-full border-b pt-6 pb-8 max-xl:px-2 bg-F7F2EB dark:bg-111216 border-gray-200 dark:border-white/5">
            <div className="w-full flex items-center justify-between">
              <h1 className="text-5xl sm:text-7xl font-extrabold tracking-wide select-none">Keşfet</h1>
              ...
              <div className="md:hidden h-8 sm:h-10">
                <button onClick={() => setMobile(!isMobileActive)}>
          ...
          <div className={classNames("relative md:hidden z-50")}>
            <div
              className={classNames(
                ...
                {
                  "opacity-50 translate-x-full": !isMobileActive,
          ...
          <div className="flex items-start">
            ...
            <div className="px-4 xl:pr-0 mt-4 w-full">
              <ShowList isLoading={isLoading} shows={Shows} onSelect={handleOnSelectShow} />
              <div
              ...
              >
                {!isMoreLoading ? (
                  ...
                  >
                    <h1 className="text-white dark:text-FFF2D7 font-bold">Daha fazla yükle...</h1>
                  </button>
                ) : (
            ...
            {isDiff && (
              <div className="fixed bottom-0 left-0 w-full bg-indigo-500 dark:bg-shark-950 shadow z-[51] sm:opacity-25 hover:opacity-100 transition-opacity duration-300">
                <button onClick={handleSubmit} className="flex items-center justify-center w-full py-4">
                  <h1 className="text-white text-xl font-extrabold uppercase">Ara</h1>
                </button>
              </div>
            )}
          </div>
        </div>
        <Modal closeable show={isModalShow} onClose={() => setIsModalShow(false)} className="sm:max-w-2xl">
          {selectedShow && (
            <div className="flex max-sm:flex-col max-sm:items-center">
              <label className="peer sm:min-w-72 sm:max-w-72 overflow-hidden shadow relative block">
                <input type="checkbox" className="peer hidden" />
                <img className="w-full" src={`https://image.tmdb.org/t/p/w780/${selectedShow?.poster_path}`} alt="movie-poster" />
              </label>
              <div className="transition duration-300 max-sm:peer-has-[:checked]:opacity-0 modal-show-info w-full flex flex-col gap-1 relative max-sm:absolute bg-white dark:bg-0F0E0E max-sm:bottom-0">
                <div className="p-2">
                  <button className="mb-2 font-thin text-3xl dark:text-white text-start w-full">{selectedShow.title || selectedShow.name}</button>
                  <div className="pt-2 max-h-40 flex overflow-auto w-full border-t dark:border-gray-500 ">
                    <h1 className="text-xs font-medium text-gray-700 dark:text-gray-300">{selectedShow.overview}</h1>
                  </div>
                </div>
                <div className="ml-2 flex flex-col text-gray-700 dark:text-gray-300 text-sm">
                  <h1>
                    <span className="mr-1">&#8226;</span>
                    {selectedShow.title ? genreIdsToNamesForMovies(selectedShow.genre_ids) : genreIdsToNamesForTV(selectedShow.genre_ids)}
                  </h1>
                </div>
                <div className="border-t dark:border-gray-700 mt-auto px-2 py-4 text-gray-700 dark:text-gray-500 text-sm flex items-center">
                  <div className="flex items-center">
                    ...
                    <p className="ms-1 text-sm font-bold text-gray-900 dark:text-gray-100">{selectedShow?.vote_average?.toFixed(1)}</p>
                    <span className="w-1 h-1 mx-1.5 bg-gray-500 rounded-full dark:bg-gray-100"></span>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{selectedShow?.vote_count} inceleme</span>
                  </div>

                  <h1 className="ml-auto">{selectedShow.release_date || selectedShow.first_air_date}</h1>
                </div>
              </div>
            </div>
          )}
