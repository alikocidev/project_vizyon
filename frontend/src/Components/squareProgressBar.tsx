...
import "@/styles/squareProgressBar.css";
...
export const SquareProgressBar: React.FC<StatusBarProps> = ({ children, occupancy, hidden, animDelay }) => {
  const color = "red";
  ...
  return (
    <>
      <div hidden={hidden} className={classNames("rounded-xl relative box1", box1OpacityColor[color])}>
        <div className="flex items-center justify-center w-full h-full box2">
          <div className="absolute z-10">{children}</div>
          <div className={classNames("rounded-xl relative box3 overflow-hidden w-full h-full", box2OpacityColor[color])}>
            <div
              className={classNames("absolute w-full bottom-0 box4 anim", inheritColor[color], afterColor[color])}
              style={{
                height: occupancy + "%",
